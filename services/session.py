...
SESSION_TTL = 60 * int(getattr(settings, "SESSION_TTL_MINUTES", 120))  # 2 hours (120 min) idle timeout
...
async def set_session(phone: str, data: dict):
    """Saves customer session state to Redis with 2 hours (120 min) TTL."""
    try:
